...
class Database:
    ...
    def update_main_file(self, container_id, filename):
        c = self.conn.cursor()
        c.execute(
            "UPDATE containers SET main_file = ? WHERE id = ?", (filename, container_id)
        )
        self.conn.commit()
...
async def stop_instance_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for bot in running_bots:
        keyboard.append(
            [InlineKeyboardButton(f"🛑 Stop: {bot[1]}", callback_data=f"stop_{bot[0]}")]
        )
...
async def server_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"🌐 Server running Python {sys.version.split()[0]}"
    )
